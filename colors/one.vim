...
if has('gui_running') || &t_Co == 88 || &t_Co == 256
  ...
  call <sid>X('LineNr',           s:mono_2,        s:syntax_cursor,  '')
  call <sid>X('CursorLineNr',     s:syntax_fg,     s:syntax_cursor,  'none')
  call <sid>X('MatchParen',       s:syntax_bg,     s:hue_3,          '')
  ...
  call <sid>X('StatusLineNormal', s:syntax_bg,     s:syntax_fg,      'none')
  call <sid>X('StatusLineReplace',s:syntax_bg,     s:hue_5_2,        'none')
  call <sid>X('StatusLineInsert', s:syntax_bg,     s:hue_2,          'none')
  ...
  if !exists("s:augroups_defined")
    ...
    function <sid>UpdateStatusLineColor(mode)
      if a:mode == 'i'
    ...
    if version >= 700
      au InsertEnter * call <sid>UpdateStatusLineColor(v:insertmode)
      au InsertChange * call <sid>UpdateStatusLineColor(v:insertmode)
      au InsertLeave * call <sid>UpdateStatusLineColor('')
    endif
